...
(function () {
    ...
    let orderNo = JSON.parse(IOT.getSessionStore(URI.SUPPLY.ORDER.DETAIL.CONTRACTMANAGEMENT.PAGE));
    let $vue = new Vue({
        ...
        mounted: function () {
            this.$nextTick(() => {
                        ...
                        },
                        {
                            ...
                            events: {
                                'click .detail-account': function (e, value, row, index) {
                                    IOT.setSessionStore(URI.SUPPLY.ORDER.DETAIL.CONTRACTMANAGEMENT.PREVIEW, JSON.stringify(row));
                                    M.Page.emit(URI.SUPPLY.ORDER.DETAIL.CONTRACTMANAGEMENT.PREVIEW);
                                    return false;
                                }
                            },
                            formatter: function (value, row, index) {
                ...
                // 查看
                if (!M.Authority.checkAuthority('product-order-create-contract-detail')) {
