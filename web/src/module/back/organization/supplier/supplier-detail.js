...
(function () {
    let storeParams = JSON.parse(IOT.getSessionStore(URI.ORGANIZATION.SUPPLIER.DETAIL.PAGE));
    let $vue = new Vue({
        el: '.supplier-detail-hook',
        data: {
        ...
        created: function () {
            IOT.getServerData(URI.ORGANIZATION.SUPPLIER.DETAIL.DATA, storeParams, (ret) => {
                if (ret.code === 200) {
                    let data = ret.data;
