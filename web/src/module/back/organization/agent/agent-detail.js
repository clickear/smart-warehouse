...
(function () {
    let storeParams = JSON.parse(IOT.getSessionStore(URI.ORGANIZATION.AGENT.DETAIL.PAGE));
    let $vue = new Vue({
        el: '.agent-detail-hook',
        data: {
        ...
        created: function () {
            IOT.getServerData(URI.ORGANIZATION.AGENT.DETAIL.DATA, storeParams, (ret) => {
                if (ret.code === 200) {
                    let data = ret.data;
