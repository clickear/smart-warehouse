        ...
        created: function () {
            IOT.getServerData(URI.RENT.RETURN.CLIENT_NEXT_RETURN.DATA, {orderNo: infoData.orderNo}, (ret) => {
                if (ret.code === 200) {
                    ...
                    this.info.returnMode = this.account.returnMode;
                    this.info.distributionMode = 1;//默认，配送方式1
                    this.info.orderNo = this.account.orderNo;
                    this.info.orderLeaseId = this.account.orderLeaseId;
                    this.info.returnType = this.account.returnType;
                    this.info.returnMode = this.account.returnMode;
                    this.info.returnAddress =this.account.pickupAddress;
                    this.info.returnAddress1 =this.account.pickupAddress;
                    $vue.$data.info.returnAddress =$vue.$data.account.pickupAddress;
                    $vue.$data.info.returnAddress1 =$vue.$data.account.pickupAddress;
                } else {
