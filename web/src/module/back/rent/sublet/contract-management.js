...
(function () {
    ...
    let orderNo = JSON.parse(IOT.getSessionStore(URI.RENT.SUBLET.DETAIL.CONTRACTMANAGEMENT.PAGE));
    let $vue = new Vue({
        ...
        mounted: function () {
            this.$nextTick(() => {
                        ...
                        },
                        {
                            ...
                            events: {
                                'click .detail-account': function (e, value, row, index) {
                                    IOT.setSessionStore(URI.RENT.SUBLET.DETAIL.CONTRACTMANAGEMENT.PREVIEW, JSON.stringify(row));
                                    M.Page.emit(URI.RENT.SUBLET.DETAIL.CONTRACTMANAGEMENT.PREVIEW);
                                    return false;
                                }
                            },
                            formatter: function (value, row, index) {
                ...
                //权限——预览
                if (!M.Authority.checkAuthority('pallet-turn-lease-contract-detail')) {
